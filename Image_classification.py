...
def train_ch3(net, train_iter, test_iter, loss, num_epochs, updater):  #@save
    ...
    train_loss, train_acc = train_metrics # 获取训练损失和训练精度
...
def predict_ch3(net, test_iter, n=6):  # @save
    """预测标签（定义见第3章）"""
    for X, y in test_iter:# 只取一个小批量进行预测
        break
    trues = d2l.get_fashion_mnist_labels(y) # 获取真实标签
    preds = d2l.get_fashion_mnist_labels(net(X).argmax(axis=1)) # 获取预测标签
    titles = [true + '\n' + pred for true, pred in zip(trues, preds)] # 标题：真实标签+预测标签
    d2l.show_images(
        X[0:n].reshape((n, 28, 28)), 1, n, titles=titles[0:n]) # 显示前n张图片及其标签
